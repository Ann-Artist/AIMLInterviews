...
38. **如何让带 DataLoader worker 的随机增强可复现？**  
...
39. **为什么增加 DataLoader worker 数量反而可能更慢？**  
...
60. **最后一个 batch 更小时，epoch loss 应如何计算？**  
...
1. **什么是 PyTorch？它与 TensorFlow 等框架有什么不同？**  
...
9. **`nn.Sequential` 和继承 `nn.Module` 有什么区别？**  
...
23. **`torchvision` transforms 用来做什么？**  
...
50. **PyTorch Lightning 如何简化 PyTorch 工作流？**  
...
1. **如何在不使用 Python 循环的情况下，对 tensor 的每一行做归一化？**
...
2. **`view`、`reshape` 和 `squeeze` 有什么区别？**
...
8. **`model.eval()` 与 `torch.no_grad()` 或 `torch.inference_mode()` 有什么区别？**
